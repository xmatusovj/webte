    ...
    if (!isAdmin($_SESSION['user'],$mysqli)) {
		...
		echo '<link rel="stylesheet" href="uloha2styly.css">';
			...
			include "config.php";
			 ...
			 $prihlaseny  = "meno1";
			 echo "Prihlaseny student s menom: " . $prihlaseny . "<hr>";
			...
			try {    
				...
				echo'
					<form action="#" method="post">
								Vyber predmet: <select name="predmetPrihlasenehoStudenta" style="width: 200px">';
				...
				echo'			
						...
						<br>
						<br>
						<input type="submit" name="vybranyPredmet" value="Vybrať predmet">
						<br>
					</form>
				';
			}
			...
			if(isset($_POST['predmetPrihlasenehoStudenta'])){
				...
				try {    
					...
					echo "<br><br>cilsloTimu:";
					echo $cilsloTimu;

					echo "<br><br>pocetBodovTimu:";
					echo $pocetBodovTimu;
			...
			//zaciatok vypisu tab
					...
					if ($prihlasenyAkoKapitanTimu == 1) {		//ak je kapitan timu	
						echo "<hr>Si kapitán tímu<hr>";
					}
					elseif ($prihlasenyAkoKapitanTimu == -1) {	//ak nie je kapitan timu
						echo "<hr>Nie si kapitán tímu<hr>";
					}			
					...
					while($row4 = $stmt4->fetch(PDO::FETCH_ASSOC)){
						echo '<tr>';
							...
							if ( $row4['suhlas'] == -1) {
								echo '<td>' . "nesúhlasí" . '</td>';
							}
							elseif ( $row4['suhlas'] == 1) {
								echo '<td>' . "súhlasí" . '</td>';
							}
							else{
								echo '<td>' . "nehlasoval" . '</td>';
							}
								...
								if($boloUzOdsuhlaseneNiekym == 0){
									...
									echo'<td>
										...
										<!-- <form action="uloha2help1.php" method="post" target="_blank"> --->
											...
											<input type="submit" value="zmeniť body">							
										</form>
									</td>';
